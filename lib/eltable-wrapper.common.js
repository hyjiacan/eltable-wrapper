...
// CONCATENATED MODULE: ./src/components/Template.vue?vue&type=template&id=fe901eea&
...
var methods = {
  ...
  remove: function remove(rows) {
    ...
    rows.forEach(function (row) {
      ...
      idx = _this.selectionData.cache.findIndex(function (row) {
        return _this.getDataId(row) === id;
      });

      _this.selectionData.cache.splice(idx, 1);
    });
    this.data.count = this.data.cache.length;
    this.pager.count = Math.ceil(this.data.count / this.pager.size);
    this.data.view = this.data.cache;
  ...
  getDataId: function getDataId(row, idField) {
    ...
    idField.forEach(function (field) {
      if (!temp.hasOwnProperty(field)) {
        _this3._throwError("Field \"".concat(idField.join('.'), "\" not found in data row, a valid \"id-field\" property is expected"));
      }
    ...
    rows.forEach(function (row) {
      ...
      if (all.hasOwnProperty(id)) {
        ...
        var idx = cache.findIndex(function (row) {
          return _this5.getDataId(row) === id;
        });
        cache.splice(idx, 1);
      }

      _this5.$refs.table.toggleRowSelection(row, false);
    });
...
var Pager_component = normalizeComponent(
  ...
  Pagervue_type_template_id_745b99ac_scoped_true_render,
  Pagervue_type_template_id_745b99ac_scoped_true_staticRenderFns,
  false,
  null,
  "745b99ac",
  null
